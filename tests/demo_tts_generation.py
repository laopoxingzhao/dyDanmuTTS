...
# -*- coding: utf-8 -*-
"""
@File    : demo_tts_generation.py
@Author  : laopo
@Time    : 2025-12-29
@Desc    : TTS音频生成演示脚本（不依赖Qt播放）
"""
...
async def demo_voice_comparison():
    ...
    # 获取所有语音选项
    from config.config import VOICE_OPTIONS
    config = Config()
    
    test_message = "这是一个语音测试，用于对比不同音色的效果"
    
    for voice_index, (voice_name, voice_info) in enumerate(VOICE_OPTIONS.items()):
        print(f"\n🎤 [{voice_index}] {voice_info['display_name']}")
        print(f"   Edge TTS: {voice_info['edge_name']}")
        ...
        try:
            ...
            print(f"   ✅ 生成成功 - {file_size:,} bytes, {generation_time:.2f}s")
            print(f"   📁 {audio_file}")
...
def print_usage():
    ...
    print("=" * 50)
    print("\n用法:")
    print("  python demo_tts_generation.py [命令]")
    print("\n命令:")
